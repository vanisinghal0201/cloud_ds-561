...
if [ ! -d "$HW8_DIR/env" ]; then
    python3 -m venv $HW8_DIR/env
    $HW8_DIR/env/bin/pip install --upgrade pip
    $HW8_DIR/env/bin/pip install -r $HW8_DIR/requirements.txt
fi
...
if ! pgrep -f "gunicorn -b 0.0.0.0:8080 main:app"; then
    $HW8_DIR/env/bin/gunicorn -b 0.0.0.0:8080 main:app --daemon
fi
