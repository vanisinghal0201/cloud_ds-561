...
def migrate_db(db: sqlalchemy.engine.base.Engine) -> None:
    ...
    with db.connect() as conn:
        conn.execute(
            sqlalchemy.text(
                    "CREATE TABLE IF NOT EXISTS Clients (user_id INT AUTO_INCREMENT PRIMARY KEY, client_ip VARCHAR(15), gender VARCHAR(10), age INT, income FLOAT, is_banned BOOLEAN);"
            )
        )
